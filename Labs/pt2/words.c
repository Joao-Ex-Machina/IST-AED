...
t_palavra  *criaPalavra(char *pal)
{
  ...
  nova -> pal = (char*)malloc(sizeof(char)*strlen(pal) +1 ) /* INSERT CODE to ALLOCATE MEMORY */;
    if(nova == NULL)
...
{
  float f= 0;
  f = p->ocorrencias/ numTotalPalavras;
  fprintf(fp,"%4d : %s, e a frequência é: %f \n", p->ocorrencias, p->pal, f);
